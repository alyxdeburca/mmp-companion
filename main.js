...
const showInit = async (origin) => {
    ...
    initBtn.onclick = async () => {
        try {
            ...
            const localBackendPath = normalizedSettings.local_backend;
            if (localBackendPath && typeof localBackendPath === "string") {
                const backendUrl = new URL(localBackendPath, origin);

                // Strict origin validation: "Verified Origin Bypass" prevention
                if (backendUrl.origin !== new URL(origin).origin) {
                    console.error("Security violation: local_backend origin mismatch.");
                    return;
                }
