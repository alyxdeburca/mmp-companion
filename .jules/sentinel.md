...
**Vulnerability:** The extension fetched configuration from a verified origin but trusted the `local_backend` value inside the fetched JSON without validation.
**Learning:** Even after verifying the source of a configuration file, the contents must still be validated. An attacker who compromises the verified source (or if the source is misconfigured) could redirect extension traffic to a malicious origin by specifying an absolute URL in the configuration.
**Prevention:** For configurable backends, resolve paths using `new URL(path, base)` and strictly validate that `backendUrl.origin` matches the verified source origin.
